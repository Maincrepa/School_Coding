...
public class Supermercat {
    public static ArrayList<String> nomsCarret = new ArrayList<String>() {{
        add("Llet (1L)");
        add("Pa de motlle");
        add("Pollastre (1kg)");
        add("Arròs (1kg)");
        add("Pomes (1kg)");
        add("Aigua mineral (6L)");
        add("Cereals");
        add("Iogurt pack x4");
    }};
    public static ArrayList<Double> preusCarret = new ArrayList<Double>() {{
        add(1.20);
        add(1.50);
        add(5.99);
        add(1.80);
    ...
    public static ArrayList<Integer> quantitatsCarret = new ArrayList<Integer>() {{
        for (int i = 0; i < nomsCarret.size(); i++) {
            add(0);
        }
    }};
    ...
    public static void veureCataleg() {
        ...
        for (int i = 0; i < nomsCarret.size(); i++) { // Mostrem tots els productes amb un bucle
            System.out.printf("    %d. %s - %.2f EUR\n", i + 1, nomsCarret.get(i), preusCarret.get(i));
    ...
    public static void afegirProducteAlCarret() {
        ...
        while (!(producte_id >= 1 && producte_id <= nomsCarret.size())) {
        ...
        quantitatsCarret.set(producte_id - 1, quantitatsCarret.get(producte_id - 1) + quantitat); // Suma la quantitat al carret (recordem que l'ID del producte PEL USUARI és 1-based i l'índex de l'array és 0-based, per aixo fem producte_id - 1)

        System.out.printf("S'han afegit %dx %s al carret!\n", quantitat, nomsCarret.get(producte_id - 1));
    ...
    public static void veureCarret() {
        ...
        for (int i = 0; i < quantitatsCarret.size(); i++) {
            // per cada producte, si la quantitat és > 0, calcula el total del producte i suma al subtotal
            if (quantitatsCarret.get(i) > 0) {

                System.out.printf(" · %s x%d | %.2f EUR\n", nomsCarret.get(i), quantitatsCarret.get(i), quantitatsCarret.get(i) * preusCarret.get(i));
    ...
    public static void eliminarProducteDelCarret() {
        ...
        if (quantitatsCarret.get(producte_id - 1) >= quantitat) {
            // ens assegurem que existeixen suficients unitats del producte al carret per eliminar la quantitat demanada

            quantitatsCarret.set(producte_id - 1, quantitatsCarret.get(producte_id - 1) - quantitat);
    ...
    public static double calcularTotalMethod() {
        ...
        for (int i = 0; i < quantitatsCarret.size(); i++) {
            if (quantitatsCarret.get(i) > 0) {
                subtotal += quantitatsCarret.get(i) * preusCarret.get(i);
            }
        }
        return subtotal;
    }
